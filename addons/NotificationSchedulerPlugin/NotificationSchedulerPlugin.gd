...
class AndroidExportPlugin extends EditorExportPlugin:
	...
	const PLUGIN_AAR_DIRECTORY = "res://addons/%s/bin" % PLUGIN_NAME
	...
	func _get_android_libraries(platform: EditorExportPlatform, debug: bool) -> PackedStringArray:
		if debug:
			return PackedStringArray(["%s/debug/%s-debug.aar" % [PLUGIN_AAR_DIRECTORY, PLUGIN_NAME]])
		else:
			return PackedStringArray(["%s/release/%s-release.aar" % [PLUGIN_AAR_DIRECTORY, PLUGIN_NAME]])
